...
if ($global.HI === void 0) {
  Reflect.defineProperty($global, "HI", Object.assign({
...
if (HI.getById === void 0) {
  const storage = Object.create(null);
  Reflect.defineProperty(HI, "getById", Object.assign({
    value(id, initialize) {
 ...
 * @description
 */
const updateQueue = $global.HI.getById(1
/* updateQueue */
, () => {
  const tasks = [];
  const pendingErrors = [];
...
const hiHTMLPolicy = $global.trustedTypes.createPolicy("hi-html", {
  createHTML: html => html
});
/* eslint-enable */

let htmlPolicy = hiHTMLPolicy;
const marker = `hi-${Math.random().toString(36).substring(2, 8)}`;
/** @internal */
...
, () => {
  ...
  function getNotifier(source) {
    let found = source.$hiController || notifierLookup.get(source);
    ...
    watch(propertySource, propertyName) {
      ...
      if (prev !== null) {
        if (!this.needsRefresh) {
          ...
          // of this watch() method. Details https://github.com/microsoft/hi/issues/4969
          let prevValue;
          watcher = void 0;
...
const contextEvent = HI.getById(3
/* contextEvent */
, () => {
  let current = null;
  return {
...
function updateContentTarget(value) {
  ...
  if (value.create) {
    ...
    let view = this.target.$hiView; // If there's no previous view that we might be able to
    ...
    if (view === void 0) {
      ...
      if (this.target.$hiTemplate !== value) {
        if (view.isComposed) {
          view.remove();
          view.unbind();
        }
    ...
    if (!view.isComposed) {
      ...
      this.target.$hiView = view;
      this.target.$hiTemplate = value;
    } else if (view.needsBindOnly) {
      view.needsBindOnly = false;
      view.bind(this.source, this.context);
    }
  } else {
    const view = this.target.$hiView; // If there is a view and it's currently composed into
...
function reduceStyles(styles) {
  return styles.map(x => x instanceof ElementStyles ? reduceStyles(x.styles) : [x]) // @ts-ignore
  .reduce((prev, curr) => prev.concat(curr), []);
}

function reduceBehaviors(styles) {
  return styles.map(x => x instanceof ElementStyles ? x.behaviors : null) // @ts-ignore
  .reduce((prev, curr) => {
    if (curr === null) {
      return prev;
    }
...
function getNextStyleClass() {
  return `hi-style-class-${++styleClassId}`;
}
/**
 * @internal
 */
...
class AttributeDefinition {
  ...
  setValue(source, newValue) {
    ...
    if (oldValue !== newValue) {
      ...
      source.$hiController.notify(this.name);
    }
  }
  /**
...
HIElementDefinition.forType = hiRegistry.getByType;
...
/**
 * Controls the lifecycle and rendering of a `HIElement`.
 * @public
 */
...
class Controller extends PropertyChangeNotifier {
   ...
   * @remarks
   * The specified element must have a {@link HIElementDefinition}
   * registered either through the use of the {@link customElement}
   * decorator or a call to `HIElement.define`.
   */


  static forCustomElement(element) {
    const controller = element.$hiController;

    if (controller !== void 0) {
      return controller;
    }

    const definition = HIElementDefinition.forType(element.constructor);

    if (definition === void 0) {
      throw new Error("Missing HIElement definition.");
    }

    return element.$hiController = new Controller(element, definition);
  }
...
function createHIElement(BaseType) {
...
const HIElement = Object.assign(createHIElement(HTMLElement), {
  /**
...
class CSSPartial extends CSSDirective {
  ...
  bind(el) {
    if (this.styles) {
      el.$hiController.addStyles(this.styles);
    }

    if (this.behaviors.length) {
      el.$hiController.addBehaviors(this.behaviors);
    }
  }

  unbind(el) {
    if (this.styles) {
      el.$hiController.removeStyles(this.styles);
    }

    if (this.behaviors.length) {
      el.$hiController.removeBehaviors(this.behaviors);
    }
  }
...
class ArrayObserver extends SubscriberSet {
  constructor(source) {
    ...
    Reflect.defineProperty(source, "$hiController", {
      value: this,
      enumerable: false
    });
  }
...
function enableArrayObservation() {
  ...
  // by another copy of hi-element.

  if (proto.$hiPatch) {
    return;
  }

  Reflect.defineProperty(proto, "$hiPatch", {
    value: 1,
    enumerable: false
  });
  const pop = proto.pop;
...
function ref(propertyName) {
  return new AttachedBehaviorHTMLDirective("hi-ref", RefBehavior, propertyName);
}
...
function slotted(propertyOrOptions) {
  ...
  return new AttachedBehaviorHTMLDirective("hi-slotted", SlottedBehavior, propertyOrOptions);
}
...
function children(propertyOrOptions) {
  ...
  return new AttachedBehaviorHTMLDirective("hi-children", ChildrenBehavior, propertyOrOptions);
}

export { $global, AttachedBehaviorHTMLDirective, AttributeDefinition, BindingBehavior, CSSDirective, ChildrenBehavior, Controller, DOM, ElementStyles, ExecutionContext, HI, HIElement, HIElementDefinition, HTMLBindingDirective, HTMLDirective, HTMLView, Observable, PropertyChangeNotifier, RefBehavior, RepeatBehavior, RepeatDirective, SlottedBehavior, SubscriberSet, TargetedHTMLDirective, ViewTemplate, attr, booleanConverter, children, compileTemplate, css, cssPartial, customElement, defaultExecutionContext, elements, emptyArray, enableArrayObservation, html, nullableNumberConverter, observable, ref, repeat, slotted, volatile, when };
